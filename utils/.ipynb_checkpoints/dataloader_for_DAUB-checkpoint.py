...
class seqDataset(Dataset):
    ...
    def get_data(self, index):
        ...
        # print('image',image_path, '%d.bmp' % image_id) ##################################################
        #print('image_id', image_id, 'label_data', label_data)
        ...
        for id in range(0, self.num_frame):
            ...
            with open(image_path + '%d.txt' % max(image_id - id, 0), 'r') as f:
                lines = f.readlines()
                # labels_box = [[int(num) for num in line.strip().split()] for line in lines] #S_DAUB!!!!!!!!!!!!
                labels_box = [[int(num) for num in line.strip().split(',')] for line in lines] #DAUB !!!!!!!!!!!!
